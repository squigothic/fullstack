...
const resolvers = {
  Query: {
    ...
    allBooks: (root, args) =>
      books
        .filter(book => (args.author ? args.author == book.author : true))
        .filter(book => (args.genre ? book.genres.includes(args.genre) : true)),
    allAuthors: () => authors,
  },
  Author: {
    titles: root => books.filter(a => a.author === root.name),
  ...
  Mutation: {
    addBook: (root, args) => {
      ...
      if (!authors.includes(args.author)) {
        console.log('args.autho: ', args.author)
        const author = { name: args.author, born: args.born, id: uuid() }
        console.log('nyt vittu author: ', author)
